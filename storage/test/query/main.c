...
void test_setup_simple_populate()
{
	...
	status = storage_set(INTTABLE, "First", &record, test_conn);
	strncpy(record.value, "col 2", sizeof record.value);
	status = storage_set(INTTABLE, "Second", &record, test_conn);
	strncpy(record.value, "col 3", sizeof record.value);
	status = storage_set(INTTABLE, "Third", &record, test_conn);
	strncpy(record.value, "col 4", sizeof record.value);
	status = storage_set(INTTABLE, "Fourth", &record, test_conn);
	strncpy(record.value, "col 5", sizeof record.value);
	status = storage_set(INTTABLE, "Fifth", &record, test_conn);
	strncpy(record.value, "col 6", sizeof record.value);
	status = storage_set(INTTABLE, "Sixth", &record, test_conn);
	strncpy(record.value, "col 7", sizeof record.value);
	status = storage_set(INTTABLE, "Seventh", &record, test_conn);
}

START_TEST (test_query_1)
{
	// Do a query.  Expect one match.
	int foundkeys = storage_query(INTTABLE, "col<6", test_keys, MAX_RECORDS_PER_TABLE, test_conn);
	fail_unless(foundkeys == 5, "Query didn't find the correct number of keys.");
}
END_TEST

START_TEST (test_query_5)
{
	// Do a query.  Expect one match.
	int foundkeys = storage_query(INTTABLE, "col > 5", test_keys, MAX_RECORDS_PER_TABLE, test_conn);
	fail_unless(foundkeys == 2, "Query didn't find the correct number of keys.");
}
END_TEST


START_TEST (test_query_2)
{
	// Do a query.  Expect one match.
	int foundkeys = storage_query(INTTABLE, "col=3", test_keys, MAX_RECORDS_PER_TABLE, test_conn);
	fail_unless(foundkeys == 1, "Query didn't find the correct number of keys.");

	char Key[1024] = "Third";
	// Check the matching keys.
	fail_unless(
		( strcmp(test_keys[0], Key) == 0 ),
		"The returned keys don't match the query.\n");
}
END_TEST


START_TEST (test_query_3)
{
	// Do a query.  Expect one match.
...
START_TEST (test_query_4)
{
	// Do a query.  Expect one match.
	int foundkeys = storage_query(INTTABLE, "col=2", test_keys, MAX_RECORDS_PER_TABLE, test_conn);
	fail_unless(foundkeys == 1, "Query didn't find the correct number of keys.");

	char Key[1024] = "Second";
	// Check the matching keys.
	fail_unless(
		( strcmp(test_keys[0], Key) == 0 ),
		"The returned keys don't match the query.\n");
...
int main(int argc, char *argv[])
{
	...
	tcase_add_test(tc, test_query_1); //added mine right here 1
	tcase_add_test(tc, test_query_2); //added mine right here 2
	tcase_add_test(tc, test_query_3); //added mine right here 3
	tcase_add_test(tc, test_query_4); //added mine right here 4
	tcase_add_test(tc, test_query_5); //added mine right here 5
