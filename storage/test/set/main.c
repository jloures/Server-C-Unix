...
START_TEST (test_set_2)
{
	...
	int status = storage_set(INTTABLE, "Second", &record, test_conn);
	fail_unless(status == 0, "Error setting a key/value pair.");
...
START_TEST (test_set_3)
{
	...
	int status = storage_set(INTTABLE, "Third", &record, test_conn);
	fail_unless(status == 0, "Error setting a key/value pair.");
...
START_TEST (test_set_4)
{
	...
	int status = storage_set(INTTABLE, "Fourth", &record, test_conn);
	fail_unless(status == 0, "Error setting a key/value pair.");
...
START_TEST (test_set_5)
{
	...
	int status = storage_set(INTTABLE, "Fifth", &record, test_conn);
	fail_unless(status == 0, "Error setting a key/value pair.");
...
int main(int argc, char *argv[])
{
	...
	tcase_add_test(tc, test_set_1); //added right here 1
	tcase_add_test(tc, test_set_2); //added right here 2
	tcase_add_test(tc, test_set_3); //added right here 3
	tcase_add_test(tc, test_set_4); //added right here 4
	tcase_add_test(tc, test_set_5); //added right here 5
