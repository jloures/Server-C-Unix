...
START_TEST (test_get_1)
{
	...
	int status = storage_set(INTTABLE, "First", &record, test_conn);
	...
	status = storage_set(INTTABLE, "", &record, test_conn);
	fail_unless(status == 0, "Error setting a key/value pair.");

	// Do a get
	strncpy(record.value, "", sizeof record.value);
	status = storage_get(SIMPLETABLES_CONF, "First", &record, test_conn);
	fail_unless(status == 0, "Error getting a value.");
	fields = sscanf(record.value, "col %d", &intval);
	fail_unless(fields == 1 && intval == 1, "Got wrong value.");
}
END_TEST

START_TEST (test_get_2)
{
	struct storage_record record;
	int fields = 0;
	int intval = 0;
	...
	// Do a get
	strncpy(record.value, "", sizeof record.value);
	status = storage_get(SIMPLETABLES_CONF, "Second", &record, test_conn);
	fail_unless(status == 0, "Error getting a value.");
	fields = sscanf(record.value, "col %d", &intval);
...
START_TEST (test_get_3)
{
	...
	// Do a get
	strncpy(record.value, "", sizeof record.value);
	status = storage_get(SIMPLETABLES_CONF, "Third", &record, test_conn);
	fail_unless(status == 0, "Error getting a value.");
	fields = sscanf(record.value, "col %d", &intval);
	fail_unless(fields == 1 && intval == 3, "Got wrong value.");
}
END_TEST

START_TEST (test_get_4)
{
	struct storage_record record;
	int fields = 0;
	int intval = 0;
	...
	// Do a get
	strncpy(record.value, "", sizeof record.value);
	status = storage_get(SIMPLETABLES_CONF, "Fourth", &record, test_conn);
	fail_unless(status == 0, "Error getting a value.");
	fields = sscanf(record.value, "col %d", &intval);
	fail_unless(fields == 1 && intval == 4, "Got wrong value.");
}
END_TEST

START_TEST (test_get_5)
{
	struct storage_record record;
	int fields = 0;
	int intval = 0;
	...
	// Do a get
	strncpy(record.value, "", sizeof record.value);
	status = storage_get(SIMPLETABLES_CONF, "Fifth", &record, test_conn);
	fail_unless(status == 0, "Error getting a value.");
	fields = sscanf(record.value, "col %d", &intval);
	fail_unless(fields == 1 && intval == 5, "Got wrong value.");
}
END_TEST
...
int main(int argc, char *argv[])
{
	...
	tcase_add_test(tc, test_get_1); //added right here 1
	tcase_add_test(tc, test_get_2); //added right here 2
	tcase_add_test(tc, test_get_3);	//added right here 3
	tcase_add_test(tc, test_get_4);	//added right here 4
	tcase_add_test(tc, test_get_5);	//added right here 5
